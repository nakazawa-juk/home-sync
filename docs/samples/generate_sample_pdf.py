...
def create_sample_schedule_pdf():
    ...
    schedule_data = [
        ...
        ["地盤調査・測量", "2025/02/01", "2025/02/05", "2025/02/01", "2025/02/04", "田中工務店", "完了", "地盤改良不要"],
        ["基礎工事", "2025/02/10", "2025/02/25", "2025/02/12", "2025/02/24", "佐藤組", "完了", ""],
        ["上棟・構造材設置", "2025/03/01", "2025/03/15", "2025/03/01", "", "木材センター", "進行中", ""],
        ["屋根工事", "2025/03/16", "2025/03/30", "", "", "屋根プロ", "未着手", ""],
        ["外壁工事", "2025/04/01", "2025/04/20", "", "", "外装工業", "未着手", "サイディング"],
        ["内装工事", "2025/04/25", "2025/05/25", "", "", "内装職人", "未着手", "クロス・フローリング"],
        ["設備工事", "2025/05/10", "2025/05/30", "", "", "設備サービス", "未着手", "電気・水道・ガス"],
        ["竣工検査・引渡し", "2025/06/01", "2025/06/05", "", "", "ABC建設", "未着手", "最終確認"]
    ]
    ...
    # 表全体の外枠
    table_rect = fitz.Rect(table_x, table_y, table_x + total_width, table_y + total_height)
    ...
    try:
        ...
        if table_list:
            ...
            if len(table_data[0]) == expected_cols:
                ...
                print(f"   ⚠️  列数不足: 期待{expected_cols}列, 実際{len(table_data[0])}列")
